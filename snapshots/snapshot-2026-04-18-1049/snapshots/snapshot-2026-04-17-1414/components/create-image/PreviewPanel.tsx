...
export function PreviewPanel({
  ...
  const previewBody = (
    <>
      <div
        role={mediaClickable ? "button" : undefined}
        tabIndex={mediaClickable ? 0 : undefined}
        className={cn(
          "relative h-full w-full",
          mediaClickable && "cursor-pointer",
        )}
        onClick={() => {
          if (mediaClickable) onPreviewClick?.();
        }}
        onKeyDown={(e) => {
          if (mediaClickable && (e.key === "Enter" || e.key === " ")) {
            e.preventDefault();
            onPreviewClick?.();
          }
        }}
