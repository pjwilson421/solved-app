...
type HistoryPanelProps = {
  ...
  onMenuEvent?: (itemId: string, event: PreviewMenuEvent) => void;
  /** Per-row liked state for the ⋮ menu heart icon. */
  previewMenuLikeActive?: (itemId: string) => boolean;
  className?: string;
  ...
  title?: string;
  /** Desktop rail only: dim thumbnail when item row is hovered. */
  thumbnailHoverOpacityOnItemHover?: boolean;
  /** Same ⋮ menu as {@link PreviewPanel} main preview. */
...
export function HistoryPanel({
  ...
  hideTitle = false,
  title = "IMAGE HISTORY",
  thumbnailHoverOpacityOnItemHover = false,
            ...
            <div className="relative">
              ...
              <div className="absolute right-2 top-2 z-20">
                <PreviewActionsMenu
                  ...
                  likeActive={previewMenuLikeActive?.(item.id) ?? false}
                  onMenuEvent={(ev) => onMenuEvent?.(item.id, ev)}
                />
              </div>
            </div>
          </div>
